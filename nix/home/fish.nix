{ pkgs, ... }:
{
  ...
  # Generated by fish itself when it upgraded to 4.3, migrating these variables
  # from universal to global scope -- but they carry real configuration: vi key
  # bindings and the full fish_color_* theme. The pure plugin supplies only
  # conf.d/_pure_init.fish and conf.d/pure.fish, so nothing else provides these.
  # Without linking them, `stow -D fish` would delete them and fish would
  # silently lose vi mode and its colours.
  #
  # Caveat: fish's own `fish_config` web tool rewrites the theme file, and it
  # cannot write through a read-only store symlink. Both files say fish would
  # rather these settings moved into config.fish -- i.e. into
  # programs.fish.interactiveShellInit here. Worth doing as its own change;
  # linking them verbatim keeps this migration behaviour-preserving.
  xdg.configFile."fish/conf.d/fish_frozen_key_bindings.fish".source =
