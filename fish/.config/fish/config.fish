...
abbr -a -- wgetr wget\ -r
abbr -a -- whoa who\ -a
abbr -a --set-cursor -- wtf gwt\ feat/dmitten/WARH-%

# Command-position abbreviations (no fish equivalent, using regular abbr)
abbr -a --set-cursor -- bugs git\ switch\ -C\ bugs/dmitten/WARH-%
abbr -a --set-cursor -- db echo\ \"export\ DB_NAME=%\"\ \>\~/.db-env\ \&\&\ source\ \~/.db-env
abbr -a --set-cursor -- feat git\ switch\ -C\ feat/dmitten/WARH-%
abbr -a --set-cursor -- manage docker\ exec\ jaguar-debug\ python\ manage.py\ %\ 2\>/dev/null
abbr -a --set-cursor -- manageh docker\ exec\ jaguar-debug\ python\ manage.py\ %\ --help\ 2\>/dev/null\ \|\ bat\ -pl\ help
abbr -a --set-cursor -- manageit docker\ exec\ -it\ jaguar-debug\ python\ manage.py\ %
abbr -a --set-cursor -- managesh docker\ exec\ -it\ jaguar-debugshell\ python\ manage.py\ %
abbr -a --set-cursor -- misc git\ switch\ -C\ misc/dmitten/%
